<?php
	...
	function crearAsignatura($conexion, $nombre_asignatura, $creditos){
		$salida = [];
		if($nombre_asignatura!="" && is_integer($creditos)){
	...
	function modificarAsignatura($conexion, $id_asignatura, $nombre_asignatura, $creditos){
		$salida = [];
			...
			is_integer($creditos))
		{
			...
			$salida["http"] = 400;
			$salida["respuesta"] = ["error" => "Error en los datos put"];
		}
	...
	function borrarAsignatura($conexion, $id_asignatura){
		if(is_numeric((int)$id_asignatura)){
			...
			// if()
			$salida["http"]=200;
			$salida["respuesta"] = ["mensaje" => "Asignatura eliminada"];
		}else{
			$salida["http"] = 400;
			$salida["respuesta"] = ["error" => "Error en los datos delete"];
		}
		return $salida;
	}
?>
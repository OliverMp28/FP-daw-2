<?php
    ...
    */
    ...
    /**
     * Crea un nuevo testimonio en la base de datos.
     *
     * @param $conexion: conexión a la base de datos
     * @param $autor: ID del socio que escribe el testimonio
     * @param $contenido: Contenido del testimonio
     * @return string: mensaje de éxito o error
     */
    function crearTestimonio($conexion, $autor, $contenido) {
        $sentencia = "INSERT INTO testimonio (autor, contenido, fecha) VALUES (?, ?, NOW())";

        $consulta = $conexion->prepare($sentencia);

        $consulta->bind_param('is', $autor, $contenido);

        if ($consulta->execute() === false) {
            return "Error al crear el testimonio: " . $consulta->error;
        }

        return "Testimonio creado exitosamente.";
    }
